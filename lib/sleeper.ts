...
// A league can have multiple drafts (mainly relevant for dynasty leagues
// with a rookie draft each year), sorted most-recent-first by Sleeper.
// Most leagues/seasons only have one.
...
export function getLeagueDrafts(leagueId: string) {
  return get<SleeperDraft[]>(`${BASE}/league/${leagueId}/drafts`);
}
